...
/* Properties getter */
/*************************/
static zda_inline zda_bool zda_avl_ht_is_empty(zda_avl_ht_t const *ht) zda_noexcept
{
  return ht->cnt == 0;
}
...
#define _zda_avl_ht_rehash_capa(ht, type, get_key, hash, cmp_cb, new_capa)                         \
  do {                                                                                             \
    zda_avl_ht_list_t *new_tb = (zda_avl_ht_list_t *)malloc(sizeof(zda_avl_ht_list_t) * new_capa); \
      ...
      for (size_t i = 0; i < ht->bkt_capa; ++i) {                                                  \
        ...
        while (root) {                                                                             \
          if (root->left) {                                                                        \
            ...
            zda_avl_tree_insert_entry_inplace(new_ht_list, entry, type, get_key, cmp_cb, p_dup);   \
            root = parent;                                                                         \
          }                                                                                        \
        }                                                                                          \
      }                                                                                            \
...
#define zda_decl_ht_insert_check(func_name, key_type, entry_type)                                  \
  entry_type *func_name(zda_avl_ht_t *ht, key_type key, zda_avl_ht_commit_ctx_t *p_ctx) zda_noexcept
