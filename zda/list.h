...
typedef struct zda_list {
  zda_list_node_t header;
} zda_list_t;

#define zda_list_node_entry(node_addr, type) container_of(node_addr, type, node)
...
static zda_inline void zda_list_node_init(zda_list_node_t *node) { node->next = node->prev = NULL; }
