...
struct MoveIfNoexcept
  : zstl::negation<zstl::conjunction<
        zstl::negation<std::is_nothrow_move_constructible<T>>,
        std::is_copy_constructible<T>>> {};

template <
    typename Iter,
    typename Dummy = zstl::enable_if_t<MoveIfNoexcept<iter_value_t<Iter>>::value>>
auto MakeMoveIteratorIfNoexcept_impl(Iter const &iter) noexcept
    -> decltype(std::make_move_iterator(iter))
{
...
auto MakeMoveIteratorIfNoexcept(Iter const &iter) noexcept
    -> decltype(MakeMoveIteratorIfNoexcept_impl(iter))
{
  return MakeMoveIteratorIfNoexcept_impl(iter);
}

#define MAKE_MOVE_ITERATOR_IF_NOEXCEPT(iter) MakeMoveIteratorIfNoexcept_impl(iter)
...
struct IsIteratorOf<
    Iter,
    Tag,
    zstl::void_t<zstl::enable_if_t<std::is_convertible<IterCategory<Iter>, Tag>::value>>>
  : std::true_type {};
...
template <typename Iter>
struct is_iterator
  : zstl::bool_constant<is_input_iterator<Iter>::value || is_output_iterator<Iter>::value> {};

template <typename Iter>
ZSTL_CONSTEXPR zstl::enable_if_t<zstl::is_iterator<Iter>::value, Iter> advance_iter(
    Iter                     it,
    zstl::IterDiffType<Iter> d
)
{
  std::advance(it, d);
  return it;
